...
import static io.leitstand.security.mac.MessageAuthenticationCodes.hmacSha512;
...
public class HmacSHA512Test {
	...
	public void setup_message_and_mac() throws Exception{
		...
		message = "message";
		mac = hmacSha512(secret).sign(message);
	}
	...
	public void accept_matching_mac(){
		assertTrue(hmacSha512(secret).isValid(message,mac));
		
	}
	
	@Test
	public void reject_mismatching_mac(){
		assertFalse(hmacSha512(new Secret(toUtf8Bytes("different_secret"))).isValid(message,mac));
	}
